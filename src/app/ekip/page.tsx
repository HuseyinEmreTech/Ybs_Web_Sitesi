...
export default async function EkipPage() {
  ...
  const diger = members.filter((m) => m.department !== 'Yönetim' && !m.role?.toLowerCase().includes('başkan'))
  ...
  return (
    <>
            ...
            {yonetim.map((member, index) => (
              <ScrollReveal key={member.id} direction="up" delay={index * 0.1}>
                <div className="bg-white dark:bg-slate-800 rounded-xl p-6 text-center shadow-sm border border-slate-200 dark:border-slate-700 hover:shadow-lg hover:-translate-y-1 transition-all">
                  ...
                  {member.bio && (
                    <p className="text-sm text-slate-600 dark:text-slate-400 mt-3 line-clamp-2">{member.bio}</p>
                  )}
                  {/* Social Links */}
                  <div className="flex justify-center gap-3 mt-4">
                    ...
                    {member.socialLinks?.github && (
                      <a href={member.socialLinks.github} target="_blank" rel="noopener noreferrer" className="text-slate-400 hover:text-slate-800 dark:hover:text-white transition-colors">
      ...
      {diger.length > 0 && (
        <Section
          title="Ekip Üyeleri"
          description="Diğer departmanlardan değerli üyelerimiz"
